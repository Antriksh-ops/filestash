...
export function useTransferSession() {
  ...
  // --- Reorder buffer for stream-based writes (StreamSaver only) ---
  const nextExpectedChunkRef = useRef(0);
  const reorderBufferRef = useRef<Map<number, ArrayBuffer>>(new Map());

  // --- Indexed chunk store for in-memory accumulation ---
  // With unordered channels, chunks arrive out of order.
  // Store by chunkId, assemble in correct order on completion.
  const chunkStoreRef = useRef<Map<number, ArrayBuffer>>(new Map());
  ...
  const processChunk = useCallback(async (data: ArrayBuffer) => {
    ...
    if (writableRef.current) {
      ...
      // Fallback: in-memory accumulation — store by chunkId (handles any arrival order)
      chunkStoreRef.current.set(chunkId, rawData);
    }
      ...
      try {
        ...
        if (meta && startTimeRef.current) {
          ...
          if (receivedSizeRef.current >= totalSize && !receiverCompleteRef.current) {
            ...
            if (!wasStreaming && meta) {
              // Assemble file from indexed chunk store
              if (chunkStoreRef.current.size > 0) {
                console.log(`[RECEIVER] Assembling file from ${chunkStoreRef.current.size} indexed chunks`);
                const sortedIds = [...chunkStoreRef.current.keys()].sort((a, b) => a - b);
                const orderedChunks: ArrayBuffer[] = [];
                for (const id of sortedIds) {
                  orderedChunks.push(chunkStoreRef.current.get(id)!);
